...
make_compare_dotplot_between_cluster <- function(object, genes.label, scale=FALSE, group.by = "res.0.6", cluster = 1, genes.use=NULL) {
...
find_markers <- function(object, group.by = "res.0.6") {
...
if (length(unique(obj@meta.data$res.0.6)) < 2) {
    quit("no")
}

dir.create(output_dir, showWarnings = F, recursive = T)

output_xlsx = paste(output_dir, "cluster_markers.xlsx", sep = "/")
...
for (i in unique(markers$ident)) {
    ...
    if (basename(input_rds) == "Alveolar_II.rds" && i %in% c(1, 2)) {
        genes.label = c(genes.label, "G6PD")
    }
    ...
    ggsave(
        filename = paste(output_dir, paste0(i, "_volca.png"), sep = "/"),
        plot = p,
        width = 6,
        height = 6,
        dpi = 600,
