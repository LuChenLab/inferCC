...
make_feature_plot <- function(data, graph, genes.use) {
    temp = data.frame(
        sc_x=graph@sc_x,
        sc_y=graph@sc_y
    )
    
    data = data[intersect(rownames(data), genes.use), ]
...
temp <- make_feature_plot(lus, graph, known_markers$gene)

plots = list()
for(i in colnames(temp)[3:ncol(temp)]) {
    data_for_plot = temp[, c("sc_x", "sc_y", i)]
    
    data_for_plot$cell = i
    colnames(data_for_plot)[3] = "value"
    
    data_for_plot = data_for_plot[order(data_for_plot$value), ]
    p <- ggplot(data = data_for_plot, aes(x=sc_x, y=sc_y, color=value)) + 
        geom_point() +
