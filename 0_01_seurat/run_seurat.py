...
class RunSeurat:
    ...
    def run_seurat_clustering(self):
        ...
        for i in os.listdir(self.input):
            args.append({
                "input": os.path.abspath(os.path.join(self.input, f"{i}/outs/filtered_feature_bc_matrix_for_seurat/")),
    ...
    def generate_report_clustering(
        ...
        for i in os.listdir(self.input):
            ...
            with open(os.path.join(out_html, i + ".html"), "w+") as w:
                w.write(self.__generate_single_page__(os.path.join(self.input, i), plots))
    ...
    def __generate_home_page__(table):
        ...
        for i in sorted([x for x in os.listdir(__static__) if os.path.isfile(os.path.join(__static__, x))]):
            if i.startswith("."):
                ...
                with open(os.path.join(__static__, i)) as r:
                    content = r.read()
                head.append(
                    f'<style type="text/css">{content}</style>\n'
                )

        for js in jss:
            with open(os.path.join(__static__, js)) as r:
                content = r.read()
            head.append(
    ...
    def __generate_single_page__(indir, plots):
        ...
        for i in sorted([x for x in os.listdir(__static__) if os.path.isfile(os.path.join(__static__, x))]):
            if i.endswith("js"):
                with open(os.path.join(__static__, i)) as r:
                    head.append(
                        f'<script type="text/javascript">{r.read()}</script>'
                    )
            else:
                with open(os.path.join(__static__, i)) as r:
                    head.append(
                        f'<style type="text/css">{r.read()}</style>'
                    )
        ...
        content = []
        for i in plots.keys():
            with open(os.path.join(indir, i), "rb") as r:
                img_string = base64.b64encode(r.read()).decode("utf-8")
            ...
            content.append(
                ...
                """ % ({
                    "img": f'<img width="90%" height="800" src="data:image/png;base64,{img_string}" />',
                    "label": plots[i]
                })
            )
