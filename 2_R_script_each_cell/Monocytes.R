...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Monocytes")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Monocytes.rds")
...
m <- get_smote_match(meta, num.cess=5340)
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1), with_disease = TRUE)
...
legend("bottomright", c("Normal", "Benign", "CPI", "I", "II", "III",'IV'), 
       col=sapply(c("Normal", "Benign", "CPI", "I", "II", "III", "IV"), function(x){colors[x]}), 
