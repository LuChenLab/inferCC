...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/CD4")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/CD4.rds")
...
m <- get_smote_match(meta, num.cells = 2841)
...
#### 5. tSNE and UMAP
pc.sd = apply(selected_obj@dr$pca@cell.embeddings, 2, sd)
...
legend("bottom", c("Normal", "CPI", "Benign", "I", "II", "III",'IV'), 
       col=sapply(c("Normal", "CPI", "Benign","I", "II", "III", "IV"), function(x){colors[x]}), 
       text.col="black", pch=16, bty="n", cex=0.8, ncol=6)
