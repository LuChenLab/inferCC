...
image_dir = here("02_figures_each_cell/Mast_SCC")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Mast.rds")
...
meta = obj@meta.data[obj@meta.data$Batch != 3 & obj@meta.data$Disease == "SCC",]
...
for (i in 1:length(stages)){
    ...
    stage = selected_obj@meta.data[selected_obj@meta.data$Stage == stages[i], ]
    p <- myDimPlot(
        selected_obj, 
        reduction.use = "tsne", 
        ...
        no.legend = FALSE,
        pt.size = 0.5,
        cols.highlight = rep(colors[stages[i]], nrow(stage))
    )
...
p <- make_combined_barplot(selected_obj, with_disease = F)
...
reducedDims(sling)$UMAP <- selected_obj@dr$umap@cell.embeddings[choosen,]
sce <- slingshot(sling, clusterLabels = 'Stage', reducedDim = 'UMAP', start.clus="I", end.clus = "III")
...
legend("topright", legends, col=cols_for_legend, 
...
legends = sort(unique(meta$Disease))
...
for(i in unique(egs$ident)) {
   temp = egs[egs$ident == i,]
   temp = do_go(temp, cluster = i)
