...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Dendritic")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Dendritic.rds")
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1), with_disease = TRUE, ncol=4)
...
legend("bottomright", c("Normal", "CPI", "Benign", "I", "II", "III",'IV'), 
       col=sapply(c("Normal", "CPI", "Benign", "I", "II", "III", "IV"), function(x){colors[x]}), 
...
legend("bottom", 
       ...
       col=disease_colors, 
       text.col="black", pch=16, bty="n", cex=1, ncol=3)
