...
change_disease_name <- function(temp) {
    temp <- sapply(temp, function(i) {
        if(i == "Normal") {
            return("Normal")
            ...
            return("LELC")
        } else if(i %in% c("Pseudotumor", "Tuberculosis")) {
            return("LBT")
        }
...
# function to format the meta info for Seurat object
chanage_meta_info <- function(object) {
    if(!"old_cluster" %in% colnames(object@meta.data)) {
        object@meta.data$old_cluster <- object@meta.data$res.0.6
    }
    
    
    object@meta.data$PatientID <- gsub("(LC?0)", "", as.character(object@meta.data$PatientID))
    object@meta.data$Stage <- as.character(object@meta.data$Stage)
    object@meta.data[object@meta.data$Batch == 3 & object@meta.data$Stage == "", "Stage"] <- "Unknown"
    object@meta.data[object@meta.data$Stage == "", "Stage"] <- "Benign"
    object@meta.data$Disease = as.character(object@meta.data$Disease)
    
    ### change info of one of the patient
    new_info = c("Disease"="CPI", "Sex"="Female", "Age"=12)
    
    for (i in c("Disease", "Sex", "Age")) {
        print(i)
        object@meta.data[, i] <- as.character(object@meta.data[, i])
        object@meta.data[object@meta.data$PatientID == "P12", i] <- new_info[i]
    }
    
    print("Tissue")
    object@meta.data[, "Tissue"] <- as.character(object@meta.data[, "Tissue"])
    object@meta.data[object@meta.data$PatientID == "P12" & object@meta.data$Tissue == "Tumor", "Tissue"] <- "CPI"
    object@meta.data[object@meta.data$Tissue == "Normal", "Stage"] <- "Normal"
    object@meta.data[object@meta.data$Tissue == "CPI", "Stage"] <- "CPI"
    
    print("left")
    # object@meta.data[object@meta.data$Disease == "", "Disease"] <- as.character(object@meta.data[obj@meta.data$Disease == "", "Tissue"])
    object@meta.data$Disease <- change_disease_name(object@meta.data$Disease)
    
    return(object)
}


# if (sd(table(meta$Stage)) > sd(table(meta$PatientID))) {
#     print("Stage")
#     
#     meta$PatientID <- as.data.frame(meta %>% select(Stage, PatientID) %>% group_by(Stage) %>% add_tally())$n
#     meta$Sex <- as.data.frame(meta %>% select(Stage, PatientID) %>% group_by(Stage) %>% add_tally())$n
#     meta$Disease <- as.data.frame(meta %>% select(Stage, Disease) %>% group_by(Stage) %>% add_tally())$n
...
#         meta[, i] <- as.numeric(meta[, i]) # meta[, i] / sum(meta[, i])
#     }
#     
#     m = SmoteClassif(Stage ~ PatientID + Disease + Sex, meta)
# } else {
#     print("PatientID")
#     
#     meta$Stage <- as.data.frame(meta %>% select(Stage, PatientID) %>% group_by(PatientID) %>% add_tally())$n
#     meta$Sex <- as.data.frame(meta %>% select(Sex, PatientID) %>% group_by(PatientID) %>% add_tally())$n
...
#     
#     m = SmoteClassif(PatientID ~ Stage + Disease + Sex, meta)
# }
# 
# meta <- na.omit(meta)
# m = na.omit(m)
...
get_smote_match <- function(meta, num.cells=NULL) {
    ...
    convert_int <- function(meta, column="Stage") {
...
make_tsne_plot <- function(object, color="Disease", alpha=0.4, colors=colors, pt.size = 0.1, guide_ncol=1) {
    ...
    p <- p + 
        ...
        theme_bw() +
        NoGrid() +
        guides(color = guide_legend(ncol=guide_ncol, override.aes = list(size = 5, alpha = 1))) +
...
make_combined_barplot <- function(
    ...
    plots = list(p1_legend, p2_legend, p3_legend, ggplot(), ggplot(), p1, p2, p3, p4, p5)
    
    if (!with_disease) {
        plots = list(p1_legend, p3_legend, ggplot(), ggplot(), p1, p3, p4, p5)
    }
...
get_entrzid <- function(markers) {
    ...
    for(i in unique(markers$ident)) {
        ...
        temp <- markers[markers$ident == i & markers$p_val_adj < 0.05, ]
