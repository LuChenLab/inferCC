...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Club_tumor")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Club.rds")
...
p <- DotPlot(selected_obj, genes.plot = unique(top10$gene), do.return = TRUE, x.lab.rot = TRUE, greoup.by = "res.0.6") + coord_flip() + theme(axis.text.x = element_text(angle = 0))
...
legend("bottomright", 
       legends,
       col=cols_for_legend, 
       text.col="black", pch=16, bty="n", cex=1, ncol=1)
