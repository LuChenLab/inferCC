...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Alveolar_II_ADC")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Alveolar_II.rds")
...
p <- make_tsne_plot(selected_obj, color="PatientID", pt.size = 0.5, guide_ncol = 2, colors=colors) + 
    theme(
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1), with_disease = FALSE)
ggsave("barplots.png", plot = p, width = 20, height = 10, dpi = dpi, units = "in", limitsize = FALSE)
...
### monocle3 with selected cells
dir.create("monocle_selected", showWarnings = F)
setwd("monocle_selected")

temp = selected_obj@dr$umap@cell.embeddings
### find out which cells needs to kept
choosen = temp[, 2] > -5 & temp[, 1] > -40
...
p <- cowplot::plot_grid(p1, p2)
ggsave(filename = "monocle3_umap_paitent_batch_selected.png", plot = p, width = 12, height = 6, dpi = 600, units = "in")
...
ggsave(filename = "monocle3_trajectory_selected.png", plot = p, width = 12, height = 6, dpi = 600, units = "in")


p <- plot_cell_trajectory(cds,alpha=0.5,color_by='Stage') + facet_grid(.~Stage)
ggsave(filename = "monocle3_trajectory_by_stage_selected.png", plot = p, width = 6 * length(unique(cds$Stage)), height = 6, dpi = 600, units = "in")
...
write.xlsx(cluster_marker_res, "monocle3_marker_genes_selected.xlsx")
...
png("monocle3_heatmap_stage_top_marker_selected.png", width = 8, height = 12, res = dpi, units = "in")
DoHeatmap(object = selected_obj, genes.use = top_marker, group.by = "Stage", slim.col.label = TRUE, remove.key = TRUE, cells.use = colnames(selected_obj@raw.data[, choosen]))
dev.off()

if (length(top_marker) > 1) {
    p <- plot_markers_by_group(cds, rev(top_marker), group_by = 'Stage', ordering_type='none')
    ggsave(filename = "monocle3_dotplot_marker_genes_selected.png", plot = p, width = 8, height = 12, dpi = 600, units = "in")
    ...
    ggsave(filename = "monocle3_umap_marker_genes_selected.png", plot = p, width = width, height = 3 * ceiling(length(top_marker) / 5), dpi = 600, units = "in")
}

saveRDS(cds, "monocle3_selected.rds")

setwd("../")

### Slingshot
dir.create("slingshot_selected")
setwd("slingshot_selected")
...
png("slingpseudotime_selected.png", width = 6, height = 6, res = dpi, units = "in")
...
for (i in grep("slingPseudotime", colnames(colData(sce)))) {
    print(i)
    png(paste0("slingpseudotime_", idx, "_selected.png"), width = 6, height = 6, res = dpi, units = "in")
...
for (i in grep("slingPseudotime", colnames(colData(sce)))) {
    ...
    topgenes <- names(sort(gam.pval, decreasing = FALSE))[1:100]
    p <- DoHeatmap(selected_obj, genes.use = topgenes, group.by = "Stage", slim.col.label = TRUE, remove.key = TRUE, cells.use = colnames(selected_obj@raw.data[, choosen]), do.plot = F)
    ggsave(paste0("slingshot_pseudotime_", idx,"_heatmap_selected.png"), width = 8, height = 12, units = "in", dpi = dpi)
    
    idx = idx + 1
}


saveRDS(sce, file = "slingshot_selected.rds")
...
registerDoMC(length(unique(markers_by_stage$ident)))
...
registerDoMC(length(unique(markers_by_cluster$ident)))
