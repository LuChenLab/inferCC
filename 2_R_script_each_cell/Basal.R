...
image_dir = here("02_figures_each_cell/Basal")
...
meta = as.data.frame(meta %>% select(PatientID, Sex, Stage, Disease))
...
n.pcs = max(which(abs(diff.pc.sd) > 0.008 * sd.difference))
...
### 
#### 1. tsne by disease
p <- make_tsne_plot(selected_obj, pt.size = 0.5, colors = disease_colors) + theme(
...
for (i in 1:length(batch)){
    ...
    ggsave(paste0("tsne_by_disease_", stages[i], ".png"), plot = p, width = 6, height = 6, units = "in", dpi = dpi)
}



#### 2. tsne by stage
p <- make_tsne_plot(selected_obj, color="Stage", colors = stage_colors, pt.size = 0.5) + 
    theme(
        legend.position = c(0.1, 0.15),
        legend.title = element_blank(),
        legend.background=element_blank()
...
for (i in 1:length(batch)){
    ...
    ggsave(paste0("tsne_by_stage_", stages[i], ".png"), plot = p, width = 6, height = 6, units = "in", dpi = dpi)
}
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1))
...
print(stages)
# the IV Stage is too less, not enough to make right prediction
sce <- slingshot(sling, clusterLabels = 'Stage', reducedDim = 'UMAP', start.clus=stages[1], end.clus = "III")
...
tempUMAP = reducedDims(sce)$UMAP
plot(tempUMAP, col = as.character(cols_for_stage[rownames(tempUMAP), "disease_colors"]), pch=16, asp = 1, cex = 0.8)
lines(SlingshotDataSet(sce), lwd = 3)
...
png("slingshot_pseudotime_1_heatmap.png", width = 8, height = 12, units = "in", res = dpi)
...
DoHeatmap(selected_obj, genes.use = top_marker, group.by = "Stage", slim.col.label = TRUE, remove.key = TRUE, cells.use = colnames(selected_obj@raw.data[, choosen]))
dev.off()


png("slingshot_pseudotime_2_heatmap.png", width = 8, height = 12, units = "in", res = dpi)
...
genes <- c("LY6D", "NTS", "C6orf141", "IL32", "UBD", "NPPC", "IER2", "SRGN", "CCL20", "JUNB")

png("featureplots_stage_I.png", width = 18, height = 6 * ceiling(length(genes) / 3), res = dpi, units = "in")
FeaturePlot(selected_obj, features.plot = genes, cols.use = c("grey", "blue"))
dev.off()
