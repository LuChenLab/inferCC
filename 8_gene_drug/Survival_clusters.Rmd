...
## Basal， differential expressed genes between ADC and SCC
```{r}
...
### further more let check if basal can compare at same stage

Only stage III have ADC and SCC at same time, give up

```{R}
temp = obj@meta.data %>% group_by(Stage, Disease) %>% add_tally() %>% select(Stage, Disease, n) %>% unique()


ggplot(data = temp, aes(x=Stage, y = log10(n), fill = Disease)) + geom_bar(stat = "identity", position = position_dodge())
```

---
### try to make scatter plots of different clusters
...
```{r}
clt = read.xlsx("/mnt/raid62/Lung_cancer_10x/02_figures_each_cell/Basal/annotation_results_by_cluster.xlsx")
