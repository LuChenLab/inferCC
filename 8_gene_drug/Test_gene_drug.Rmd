...
full.path <- function(path) {
    return(paste("/mnt/raid62/Lung_cancer_10x/07_gene_drug", path, sep = "/"))
}
```
...
meta2 <- read.xlsx(full.path("gene_drug.xlsx"))
```
...
DotPlot(
    ap2, 
    genes.plot = intersect(rownames(ap2@raw.data), sort(c("G6PD", "SERPINB9", meta2$Symbol))),
    group.by = "res.0.6",
    x.lab.rot = T
)
...
DotPlot(
    basal, 
    genes.plot = intersect(rownames(basal@raw.data), sort(c("G6PD", "SERPINB9", meta2$Symbol))),
    group.by = "res.0.6",
    x.lab.rot = T
)
```
...
p = FeaturePlot(
    ap2, 
    features.plot = intersect(rownames(ap2@raw.data), sort(c("G6PD", "SERPINB9", meta2$Symbol))),
...
p = FeaturePlot(
    basal, 
    features.plot = intersect(rownames(basal@raw.data), sort(c("G6PD", "SERPINB9", meta2$Symbol))),
...
```{r}
ap2_clt[ap2_clt$avg_logFC > 0 & ap2_clt$p_val_adj < 0.05 & ap2_clt$gene %in% sort(c("G6PD", "SERPINB9", meta2$Symbol)), ]
```

```{r}
basal_clt[basal_clt$avg_logFC > 0 & basal_clt$p_val_adj < 0.05 & basal_clt$gene %in% sort(c("G6PD", "SERPINB9", meta2$Symbol)), ]
```
...
###### APII
```{R fig.height=6, fig.width=12}
for(i in sort(c("G6PD", "SERPINB9", meta2$Symbol))) {
...
```{r fig.height=16, fig.width=16}
VlnPlot(ap2, features.plot = intersect(rownames(ap2@raw.data), c("G6PD", "SERPINB9", meta2$Symbol)[1:20]), group.by = "res.0.6", do.sort = T)
```

```{r fig.height=16, fig.width=16}
VlnPlot(basal, features.plot = intersect(rownames(basal@raw.data), c("G6PD", "SERPINB9", meta2$Symbol)[1:20]), group.by = "res.0.6", do.sort = T)
```
...
msig <- tmodImportMSigDB("/mnt/raid62/Lung_cancer_10x/07_msigdb_v6.2.xml")
...
saveWorkbook(wb, "/mnt/raid62/Lung_cancer_10x/07_gene_drug/APII_tmod.xlsx", overwrite = T)
```
...
saveWorkbook(wb, "/mnt/raid62/Lung_cancer_10x/07_gene_drug/Basal_tmod.xlsx", overwrite = T)
```
...
ggsave(
    "/mnt/raid62/Lung_cancer_10x/07_gene_drug/APII_G6PD.png",
    plot = p,
    width = 18,
    height = 6,
    dpi = 600,
...
ggsave(
    "/mnt/raid62/Lung_cancer_10x/07_gene_drug/Basal_G6PD.png",
    plot = p,
    width = 18,
    height = 6,
    dpi = 600,
...
p
ggsave(
    "/mnt/raid62/Lung_cancer_10x/07_gene_drug/APII_tmod_1.png",
    plot = p,
    width = 12, height = 8,
    dpi = 600,
    units = "in"
...
ggsave(
    "/mnt/raid62/Lung_cancer_10x/07_gene_drug/APII_tmod_2.png",
    plot = p,
    width = 12,
    height = 8,
    dpi = 600,
...
ggsave(
    "/mnt/raid62/Lung_cancer_10x/07_gene_drug/Basal_tmod_10.png",
    plot = p,
    width = 12, height = 8,
    dpi = 600,
    units = "in"
...
---
Try to make huge gene expression plots
```{r}
scale_data <- read.csv("/mnt/raid62/Lung_cancer_10x/figures/VlnPlot_gene_drug/scale.csv", row.names = 1)
meta_data <- read.csv("/mnt/raid62/Lung_cancer_10x/figures/VlnPlot_gene_drug/meta.csv", row.names = 1)
```
