...
root.dir = "/mnt/raid62/Lung_cancer_10x/02_figures_each_cell/B_cells_ADC"
rds = "/mnt/raid62/Lung_cancer_10x/02_figures_each_cell/B_cells_ADC/B_cells.rds"
...
cl = perform_mfuzz(expr, cluster_markers, logfc=logfc, qvalue=qvalue, init_cluster = max(obj@meta.data$res.0.6))

mtx = construct_stage_group_zscore(obj, expr, cluster=cl)

temp = hclust(dist(mtx))
make_heatmap(obj, cl$cluster, group.by = "res.0.6")
plot(temp)
...
make_heatmap(obj, new_cl, group.by = "res.0.6")
make_dotplot(obj, new_cl)


make_heatmap(obj, new_cl, group.by = "res.0.6", output_prefix = paste(root.dir, "cluster_gene_module/heatmap_", sep = "/"))
make_dotplot(obj, new_cl, group.by = "res.0.6", output_prefix = paste(root.dir, "cluster_gene_module/dotplot_", sep = "/"))
