...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Exhaust_T")
input_rds = here("00_data_ingest/04_rds_generated/seurat_obj/Exhaust_T.rds")
...
markers <- c(
    "CCL4L2", "CD8A", "CD8B", "GZMK", "LAYN", "LEF1", 
    "PDCD1", "CD274", "TIGIT", "LAG3", "TIM3", "GTLA4",
    "CD44", "CD43", "CD69", "CD127", "CD62L"
    )

markers <- intersect(markers, rownames(selected_obj@raw.data))

png("heatmap_markers.png", width = 8, height = 12, res = dpi, units = "in")
DoHeatmap(object = selected_obj, genes.use = markers, slim.col.label = TRUE, remove.key = TRUE, group.by="res.0.6")
dev.off()


p <- DotPlot(selected_obj, genes.plot = markers, do.return = TRUE, x.lab.rot = TRUE, group.by="res.0.6") + coord_flip() + theme(axis.text.x = element_text(angle = 0))
ggsave("dotplot_markers.png", p, dpi = dpi, width = 6, height = 12, units = "in")


p = FeaturePlot(selected_obj, features.plot = markers, cols.use = c("grey", "blue"), reduction.use = "tsne", do.return = TRUE)
p <- customize_Seurat_FeaturePlot(p, gradient.use = c("grey", "blue"))

p <- plot_grid(plotlist = p, ncol = 3, nrow = ceiling(length(markers) / 3))

ggsave("features.png", plot = p, width = 18, height = ceiling(length(markers) / 3) * 6, units = "in", dpi=dpi)
...
for (i in 1:length(batch)){
    ...
    ggsave(paste0("tsne_by_disease_", batch[i], ".png"), plot = p, width = 6, height = 6, units = "in", dpi = dpi)
}
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1), with_disease = T, ncol=4)
...
p <- DotPlot(selected_obj, genes.plot = unique(top10$gene), do.return = TRUE, x.lab.rot = TRUE, group.by="res.0.6") + coord_flip() + theme(axis.text.x = element_text(angle = 0))
...
sce <- slingshot(sling, clusterLabels = 'Stage', reducedDim = 'UMAP', start.clus="Normal", end.clus = "IV")


legends = c("Normal", "CPI", "Benign", "I", "II", "III",'IV')
