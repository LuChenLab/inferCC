...
# if the image direcotory not exists, create it
image_dir = here("02_figures_each_cell/Alveolar_II_Tumor")
...
meta = obj@meta.data[obj@meta.data$Batch != 3 & obj@meta.data$Stage %in% c("I", "II", "III", "IV"),]
...
#### 2. tsne by stage
p <- make_tsne_plot(selected_obj, color="Stage", colors=colors, pt.size = 0.5) + 
    theme(
        legend.position = c(0.9, 0.8),
        legend.title = element_blank(),
        legend.background=element_blank(),
...
p <- make_combined_barplot(selected_obj, rel_height = c(0.3, 1), with_disease = TRUE, ncol=5)
...
# choosen = colnames(selected_obj@raw.data[, choosen])
# choosen = rownames(selected_obj@meta.data[choosen, ][selected_obj@meta.data$Stage != "Benign",])
...
ggsave(filename = "monocle3_trajectory_by_stage.png", plot = p, width = 6 * length(unique(selected_obj@meta.data$Stage)), height = 6, dpi = 600, units = "in")
...
png("monocle_heatmap_stage_top_marker.png", width = 8, height = 12, res = dpi, units = "in")
...
sce <- slingshot(sling, clusterLabels = 'Stage', reducedDim = 'UMAP', start.clus=stages[1], end.clus = "IV")
...
stages = sort(unique(selected_obj$Stage))

legend("topright", stages, 
       col=sapply(stages, function(x){colors[x]}), 
...
for (i in grep("slingPseudotime", colnames(colData(sce)))) {
    ...
    p <- DoHeatmap(
        ...
        slim.col.label = TRUE, 
        remove.key = TRUE, 
        cells.use = colnames(selected_obj@raw.data[, choosen]),
        do.plot = F
    )
    ggsave(paste0("slingshot_pseudotime_", idx,"_heatmap.png"), plot = p, width = 8, height = 12, units = "in", dpi = dpi)
