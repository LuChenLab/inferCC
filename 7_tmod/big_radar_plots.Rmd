...
full.path <- function(path) {
    return(paste("/mnt/raid62/Lung_cancer_10x/05_tmod_stage/scale_mean_for_radar", path, sep = "/"))
}
```
...
```{R}
res = NULL
for(i in unique(et_adc_de$ident)) {
    temp = et_adc_de[et_adc_de$ident == i, ]
    
    temp = tmodUtest(rownames(temp[order(temp$logFC), ]), mset=msig[sel], qval = Inf)
    
    res = rbind(res, temp)
}

res <- res[res$AUC > 0.5 & res$adj.P.Val < 0.05, ]
```

```{R}

mat = ExpressionSet(as.matrix(np_expr_scc))
mat = standardise(mat)

cl <- mfuzz(mat, c=16, m=mestimate(mat))
```
