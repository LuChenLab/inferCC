...
```{r}
sysu_luad <- read.csv("SYSU/ADC.csv", row.names = 1)
sysu_lusc <- read.csv("SYSU/SCC.csv", row.names = 1)
```


## Read TCGA data
```{r}
tcga_luad = readRDS("TCGA/LUAD.rds")
tcga_lusc = readRDS("TCGA/LUAD.rds")
```
...
files = glob$glob("03_each_cells/LUSC/*/mfuzz/results.csv")

registerDoMC(20)
for(i in files) {
    print(i)
    # if(file.exists(paste(output_dir, "mfuzz_radar_bulk.pdf", sep = "/"))) {
    #     next
    # }
...
```{r}
files = glob$glob("03_each_cells/LUAD/*/mfuzz/results.csv")
