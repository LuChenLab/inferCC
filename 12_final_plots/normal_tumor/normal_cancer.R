...
print(table(obj@meta.data$Disease))

for (i in c("LUSC", "LUAD")) {
    # print(paste(i, "Normal", sep = "_"))

    normal_cells = rownames(obj@meta.data)[obj@meta.data$Disease == paste(i, "Normal", sep = "_")]
    ...
    if (length(normal_cells) > 10 && length(disease_cells) > 10) {
        ...
        ggsave(
            filename = paste(output_dir, paste0(i, "_voca.pdf"), sep = "/"),
            plot = p,
            width = 6,
            height = 6,
            dpi = 600,
